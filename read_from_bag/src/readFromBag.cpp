...
int main()
{
	int mode = 1; // 1 for GPS/IMU, 2 for velodyne, 3 for left image, 4 for right image, 5 for INSPVAX
    ...
    string IMUGPSFilePath = IMUGPSFolderPath + "imugps_record.txt";
        ...
        if(mode == 1)
        {
        	...
        	if (imu_data != NULL)
        	{
            	...
            	if(gpsIsSet)
            	{
                    ...
                    sprintf(stemp, "%6.20f %6.20f %6.20f %6.20f",
                       server_time,
